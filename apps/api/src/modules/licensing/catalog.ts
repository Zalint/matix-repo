...
export const MODULE_CATALOG: ModuleDefinition[] = [
  ...
  { code: 'platform.integrations',     pillar: 'platform', label: { fr: 'Intégrations tierces',    en: 'Third-party integrations' }, actions: ['read','write','delete'], status: 'coming-soon', description_fr: 'Hub centralisé d\'intégrations tierces (Gmail, Bictorys, Wave, Orange Money, Slack, OpenAI). Stocke credentials sécurisés, gère webhooks entrants/sortants, normalise les payloads. Évite que chaque module ré-implémente la connexion à chaque service tiers.' },
  { code: 'platform.workflows',        pillar: 'platform', label: { fr: 'Workflows / automations',  en: 'Workflows / automations' }, actions: ['read','write'], status: 'coming-soon', description_fr: 'Workflows pré-configurés par Matix (3 templates au démarrage : daily_cash_report, daily_mlc_report, daily_business_agent). Chaque tenant reçoit une instance clonée de chaque template à l\'activation du module. Le superviseur peut PARAMÉTRER (heure cron, destinataires email, seuils) mais NE PEUT PAS créer de nouveau workflow ni modifier la logique. Pour un workflow custom : ticket auprès de l\'admin Matix qui ajoute un nouveau template au catalogue. Engine sous-jacent : n8n (caché derrière UI Matix, pas exposé aux tenants).' },
  ...
  { code: 'commercial.sales.reconciliation',   pillar: 'commercial', label: { fr: 'Réconciliation ventes',    en: 'Sales reconciliation' },actions: ['read','write'],          status: 'active', description_fr: 'Vue quotidienne des écarts (stock soir saisi/calculé vs théorique) avec notes par (date, PV). S\'appuie sur la gestion stock (modes manuel/automatique) — voir operations.inventory.movements.' },
  ...
  { code: 'analytics.reports.daily_digest',    pillar: 'analytics', label: { fr: 'Rapport quotidien digest', en: 'Daily digest report' },   actions: ['read','write'],          status: 'coming-soon', description_fr: 'Rapport quotidien automatique envoyé par email. Agrège les KPIs clés (CA, écarts stock, créances, virements, dépenses, livreurs) depuis plusieurs modules Matix. Configurable : heure d\'envoi, destinataires, sources, format. Pattern issu des 3 workflows n8n actuels (MATA REPORT 1/2 + MATA BANQ).' },
  ...
  { code: 'analytics.ai.agent',                pillar: 'analytics', label: { fr: 'Agent IA conversationnel',  en: 'Conversational AI agent' }, actions: ['read'],                  status: 'coming-soon', description_fr: 'Agent IA avec accès aux APIs Matix. Permet de demander en langage naturel : "CA aujourd\'hui ?", "meilleur livreur cette semaine ?", "client avec plus grosse créance ?". Architecture : LLM + outils HTTP qui appellent les modules Matix selon le besoin. Inspiré du webhook MATA AGENT n8n actuel (16 APIs agrégées).' },
